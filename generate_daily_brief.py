...
【白話、通俗、精闢文體硬性規範】：
1. 文體風格：絕對不要寫成生硬的學術論文、驗收報告或公文！請用「白話、通俗、精闢、極好閱讀」的對話口吻，就像在晨間咖啡時間跟朋友分享有趣的好東西。
2. 開門見山：用直白的日常生活場景或痛點開場，講清楚「這件事為什麼跟你有關、價值在哪裡」，去掉所有饒舌的高深術語。
3. 每日標題獨特性 (Headline)：`headline` 必須依據當天的關鍵動態，寫出吸引人且易讀的大標題（如「手作拼貼美學復興、AI 拿不走你的品味與無摩擦政府」），絕對禁止使用固定套版範本！
4. 比例控管：每日兩大領域中，AI 相關議題「各佔 2 ~ 3 則為上限」，絕對不畫地自限只寫 AI！
5. 🎨 設計與美學板塊 (5 則)：2~3 則平面/動態視覺藝術與字體美學，2 則 AI UI/UX 人機協作。
6. 🏛️ 公共治理與社會創新板塊 (5 則)：2~3 則政治性實驗與 DPI 白話服務，2 則 AI 治理問責。
7. 網域多樣性與真實性：10 則新聞必須來自 10 個完全不同的權威機構與網域，且連結全數為實時可點開的深層 URL。
